...
/* Timer used to send notification value */
PRIVILEGED_DATA static OS_TIMER cs_timer;
...
static const cs_parameter_t cs_pms = {
        .custom_msg = "Greetings Traveler!",
};
...
static void cs_msg_write_cb(ble_service_t *svc, uint16_t conn_idx, const uint8_t *value)
{

}

static void cs_notif_changed_cb(uint16_t conn_idx, bool enabled)
{
        if (enabled) {
                OS_TIMER_START(cs_timer, OS_TIMER_FOREVER);
        } else {
                OS_TIMER_STOP(cs_timer, OS_TIMER_FOREVER);
        }
}

static cs_callbacks_t cs_callbacks =
{
        .received_data=cs_msg_read_cb,
        .send_data=cs_msg_write_cb,
        .notif_changed=cs_notif_changed_cb,
};
...
static void hrp_timer_cb(OS_TIMER timer)
{
        ...
        OS_TASK_NOTIFY(task, CS_TIMER_NOTIF, OS_NOTIFY_SET_BITS);
}
...
void ble_peripheral_task(void *params)
{
        ...
        ble_service_t* cs_service = chat_service_init(&cs_pms,&cs_callbacks);

        /*
         * Create timer for HRS which will be used to send measurement every 1 second once client
         * is connected.
         */
        cs_timer = OS_TIMER_CREATE("hrs", 1000 / OS_PERIOD_MS, OS_TIMER_SUCCESS,
...
no_event:
                ...
                /*notified from CS timer? */
                if (notif & CS_TIMER_NOTIF)
                {

                       // static const char *not_msg = "notified";
                        ...
                        cs_notify(cs_service, active_conn_idx, payload);
                        printf(NEWLINE "Value Notified to client: %d"NEWLINE ,*payload);
                        fflush(stdout);
